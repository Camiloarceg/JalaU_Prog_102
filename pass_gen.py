...
if __name__ == "__main__":
    ...
    while True:
        # app name request
        app_name = input("Please enter your app name: ").lower()
        ...
        if has_data_rows(path):
            ...
            # if app is already take a decision
                ...
                decision = input(
                    "Seems you already have a password, 1 for view password or 2 for overwrite password: "
                )
                if decision == "1":
                    # print the retieved password
                    print(
                        f"Your password for {app_name} is {get_password(path, app_name)}"
                    )
                else:
                    # overwrite the password
                    char_num = character_assignation()
